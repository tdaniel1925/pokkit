...
export default function ConfigurePage({
  ...
  const handleCreateLife = async () => {
    ...
    try {
      ...
      // Navigate to the first observation page (still in ritual mode)
      router.push(`/world/${worldId}/observe`);
    } catch (err) {
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#0a1f0a] text-[#f5f5f0]">
      <div className="h-full overflow-y-auto">
        <div className="max-w-2xl mx-auto px-8 py-16">
          {/* Header */}
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-5xl font-light mb-4">
              Configure Your Creation
            </h1>
          ...
          {/* World Name */}
          <div className="mb-12">
            <label className="block text-sm text-[#f5f5f0]/60 mb-3">
              Name your world
            </label>
            <input
              type="text"
              ...
              className="w-full bg-transparent border-b border-[#f5f5f0]/30 focus:border-[#f5f5f0] px-0 py-3 text-2xl text-[#f5f5f0] placeholder:text-[#f5f5f0]/30 focus:outline-none transition-colors"
            />
          </div>
          ...
          <div className="mt-16 text-center pb-8">
            <button
              ...
              className="group relative px-12 py-4 text-xl font-light border border-[#f5f5f0]/50 hover:border-[#f5f5f0] hover:bg-[#f5f5f0] hover:text-[#0a1f0a] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isCreating ? (
  ...
  return (
    <div>
      <div className="flex justify-between items-baseline mb-2">
        ...
        <span className="text-[#f5f5f0]/80">{format(value)}</span>
      </div>
      <input
        type="range"
        min={min}
        ...
        className="w-full h-1 bg-[#f5f5f0]/20 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#f5f5f0]"
      />
      <p className="text-[#f5f5f0]/40 text-sm mt-2">{description}</p>
    </div>
  );
}
